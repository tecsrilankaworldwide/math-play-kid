...
export default function NumbersModule() {
  ...
  const fetchQuestion = async () => {
    ...
    setIsCorrect(null);
    try {
      const response = await axios.get(`${API}/question/numbers`);
  ...
  useEffect(() => {
    fetchQuestion();
    axios.get(`${API}/progress`).then(res => setStars(res.data.numbers_stars || 0));
  }, []);
  ...
  const handleAnswer = async (answer) => {
    ...
    if (correct) {
      ...
      try {
        ...
        setStars(res.data.numbers_stars);
      } catch (e) {

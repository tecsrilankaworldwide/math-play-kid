...
export default function CountingModule() {
  ...
  const fetchQuestion = async () => {
    ...
    setIsCorrect(null);
    try {
      const response = await axios.get(`${API}/question/counting`);
  ...
  useEffect(() => {
    fetchQuestion();
    // Get current stars
    axios.get(`${API}/progress`).then(res => setStars(res.data.counting_stars || 0));
  }, []);
  ...
  const handleAnswer = async (answer) => {
    ...
    if (correct) {
      ...
      try {
        ...
        setStars(res.data.counting_stars);
      } catch (e) {
  ...
  const renderObjects = () => {
    ...
    return (
      <div className="objects-grid py-6">
